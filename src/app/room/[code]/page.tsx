...
type RoomPageProps = {
  ...
  searchParams: Promise<{ name?: string; rounds?: string; mode?: string; guesser?: string }>;
};

function parseRoomSettings(query: {
  rounds?: string;
  mode?: string;
  guesser?: string;
}): Partial<RoomSettings> | undefined {
  if (!query.rounds && !query.mode && !query.guesser) {
    return undefined;
  }

  return {
    totalRounds: normalizeTotalRounds(query.rounds),
    gameMode: normalizeGameMode(query.mode === "fixed" ? "fixed_guesser" : query.mode),
    fixedGuesser: normalizeFixedGuesser(query.guesser),
  };
}
...
export default async function RoomPage({ params, searchParams }: RoomPageProps) {
  ...
  const roomSettings = parseRoomSettings(query);
